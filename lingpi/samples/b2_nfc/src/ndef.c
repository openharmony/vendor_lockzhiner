...
static int16_t firstRecord(UncompletePageStr *page, const NDEFDataStr *data, RecordPosEnu rtdPosition)
{
    ...
    // clear all buffers
    memset_s(&record, sizeof(record), 0, sizeof(NDEFRecordStr));
    ...
    // write first record
    memset_s(nfcPageBuffer, sizeof(nfcPageBuffer), &header, sizeof(NDEFHeaderStr));
...
static int16_t addRecord(UncompletePageStr *pageToUse, const NDEFDataStr *data, RecordPosEnu rtdPosition)
{
    ...
    memset_s(&record, sizeof(record), 0, sizeof(NDEFRecordStr));
    memset_s(tmpBuffer, sizeof(tmpBuffer), 0, NFC_PAGE_SIZE);
    ...
    if (pageToUse->usedBytes + recordLength < NFC_PAGE_SIZE) {
        memset_s(&nfcPageBuffer[pageToUse->usedBytes],
            sizeof(nfcPageBuffer[pageToUse->usedBytes]),
            tmpBuffer,
            recordLength);
        ...
        memset_s(nfcPageBuffer, sizeof(nfcPageBuffer), &tmpBuffer[byteToCopy], pageToUse->usedBytes);
...
static bool writeUserPayload(int16_t payloadPtr, const NDEFDataStr *data, UncompletePageStr *addPage)
{
    ...
    // Copy the payload
    memcpy_s(&nfcPageBuffer[payloadPtr], sizeof(nfcPageBuffer[payloadPtr]), data->rtdPayload, copyByte);
    ...
    while (!endRecord) {
        ...
        if (addedPayload < data->rtdPayloadlength) {
            ...
            if ((data->rtdPayloadlength - addedPayload) < NFC_PAGE_SIZE) {
                memcpy_s(nfcPageBuffer,
                    sizeof(nfcPageBuffer),
                    &data->rtdPayload[addedPayload],
